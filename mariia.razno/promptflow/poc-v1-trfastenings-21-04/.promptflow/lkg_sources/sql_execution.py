...
def not_unique_columns(rows, cursor):
    ...
    variation_dict = {key: val for key, val in variation_dict.items() if key not in['Web Reference']}
...
def create_table_from_df(df, table_name, cursor):
    ...
    if table_name != 'product_catalogue':
...
def links_extraction(df, cursor):
    web_reference = list(df['Web Reference'])
    formatted_references = ', '.join(f"'{item}'" for item in web_reference)
    select_query = f"""SELECT [Web Reference], [finish_name] FROM [finish_links] WHERE [ref_num] IN ({formatted_references});"""
    cursor.execute(select_query)
    rows = cursor.fetchall()
    links = df_from_extracted_products(rows, cursor)
    ref_dict = {}
    for ind, elem in links.iterrows():
        ref_dict[elem['Web Reference']] = elem['finish_name']
    return ref_dict
...
def query_execution(query1, query2, grounding):
    ...
    if num_products == 0:
        ...
        cursor.close()
        conn.close()
        return {'num_products': num_products, 'num_all': num_all, 'table_name': table_name, 'not_unique': not_unique, 'links': links, 'sql': sql, 'result': rows}
        ...
        conn.commit()
        # Extract links for chosen products
        links = links_extraction(df, cursor)
        # close connection
        cursor.close()
        conn.close()
        return {'num_products': num_products, 'num_all': num_all, 'table_name': table_name, 'not_unique': not_unique, 'links': links, 'sql': sql, 'result': rows}
    ...
    # IF number of extracted products is still huge but less than previously extracted
        ...
        cursor.close()
        conn.close()
        return {'num_products': num_products, 'num_all': num_all, 'table_name': table_name, 'not_unique': not_unique, 'links': links, 'sql': sql, 'result': rows[:5]}
