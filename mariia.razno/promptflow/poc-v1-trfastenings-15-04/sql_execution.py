...
def not_unique_columns(rows, cursor):
    ...
    # Analyze the DataFrame to find columns with different values
    columns_with_variation = [col for col in df.columns if df[col].nunique() > 1]
    columns_with_variation_str = ", ".join(columns_with_variation)
    return columns_with_variation_str
