...
def not_unique_columns(rows, cursor):
    ...
    variation_dict = {key: val for key, val in variation_dict.items() if key not in['Web_Reference']}
...
def query_execution(query1, query2, grounding):
    ...
    # IF number of extracted products is still huge but less than previously extracted
        ...
        cursor.close()
        conn.close()
        return {'num_products': num_products, 'num_all': num_all, 'table_name': table_name, 'not_unique': not_unique, 'sql': sql, 'result': rows[:5]}
